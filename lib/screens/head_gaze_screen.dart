/// ===========================================================================
/// GazeNav v6.0 - HEAD TRACKING + SYSTEM NAVIGATION
/// ===========================================================================
///
/// Two phases:
///   Phase 1: CALIBRATION (in-app) → user calibrates + tests cursor
///   Phase 2: NAVIGATION  (overlay) → user goes to home screen with
///            navigator pad overlay, cursor, and gesture controls
///
/// On "RUN APP" → checks accessibility service → starts overlay →
/// sends user to home screen → continues camera processing in background
/// and sends cursor position to native overlay via platform channel.
///
/// ===========================================================================
...
class _HeadGazeScreenState extends State<HeadGazeScreen>
  ...
  // ── App state ──
  bool _overlayRunning = false;     // True when overlay is active on home screen
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.inactive) {
      // DON'T stop camera if overlay is running - we need it for tracking
      if (!_overlayRunning) {
        _camCtrl?.stopImageStream();
      }
  ...
  // ══════════════════════════════════════════════════
  // RUN APP (Start overlay + go to home screen)
  // ══════════════════════════════════════════════════
  ...
  Future<void> _startNavigation() async {
    ...
    try {
      ...
      debugPrint('NAV: Overlay started, going to home screen');
    } catch (e) {
      debugPrint('NAV: Failed to start overlay: $e');
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _stopNavigation() async {
    try {
      ...
      debugPrint('NAV: Failed to stop overlay: $e');
    }
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'GazeNav needs the Accessibility Service to perform gestures '
              'on your home screen (swipes, taps).\n\n'
              'Please enable "GazeNav" in:\n'
              'Settings → Accessibility → Installed Services → GazeNav',
  ...
  Future<void> _processFrame(CameraImage image) async {
    try {
      ...
      if (tracked) {
        ...
        // If overlay is running, send cursor to native
        if (_overlayRunning) {
          _sendCursorToNative(_cursorPos.dx, _cursorPos.dy);
        }
      }
  ...
  void _sendBlinkToNative() {
    try {
      _channel.invokeMethod('doubleBlink');
    } catch (_) {}
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: _headTracker.isCalibrated
                            ? [
                          BoxShadow(
                            color: (_overlayRunning
                                ? Colors.redAccent
                                : Colors.cyan)
                                .withOpacity(0.3),
                            blurRadius: 20,
          ...
          if (_headTracker.calibrationPhase == CalibrationPhase.center &&
              _faceDetected)
            _buildCalTarget(
                screenSize.width / 2, screenSize.height / 2, 'Look here'),
          if (_headTracker.calibrationPhase == CalibrationPhase.rangeLeft &&
              _faceDetected)
            _buildCalTarget(40, screenSize.height / 2, 'Look LEFT'),
          if (_headTracker.calibrationPhase == CalibrationPhase.rangeRight &&
              _faceDetected)
            _buildCalTarget(
                screenSize.width - 40, screenSize.height / 2, 'Look RIGHT'),
          if (_headTracker.calibrationPhase == CalibrationPhase.rangeUp &&
              _faceDetected)
            _buildCalTarget(screenSize.width / 2, 80, 'Look UP'),
          if (_headTracker.calibrationPhase == CalibrationPhase.rangeDown &&
              _faceDetected)
            _buildCalTarget(
                screenSize.width / 2, screenSize.height - 120, 'Look DOWN'),
          ...
          // ── In-app cursor (calibration mode only) ──
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                        'Pos: (${_cursorPos.dx.toInt()}, ${_cursorPos.dy.toInt()})',
                        style: _debugStyle),
                    Text('FPS: $_fps  Overlay: $_overlayRunning',
                        style: _debugStyle),
                    Text(
                      'Eyes: ${_blinkDetector.rawProbability.toStringAsFixed(2)} '
                          '[${_blinkDetector.stateLabel}]',
                      style: _debugStyle,
                    ),
                    Text(
                      'Blinks: ${_blinkDetector.totalBlinks} '
                          'Dbl: ${_blinkDetector.totalDoubleBlinks} '
                          'Long: ${_blinkDetector.totalLongBlinks}',
                      style: TextStyle(
                        color: (_blinkDetector.totalDoubleBlinks +
                            _blinkDetector.totalLongBlinks) >
                            0
                            ? Colors.greenAccent
                            : Colors.white70,
                        fontSize: 11,
